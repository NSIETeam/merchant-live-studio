...
| 定稿确认     | 确认时间、当前商家身份、核对说明                     | 生产及团队空间要求独立账号审核；本地单人演示保留自确认     |
...
6. 团队和生产空间由编辑提交具体版本，再由另一审核账号填写意见、批准或退回。退回后建立新版本重新提交；单人本地演示才保留本人确认。服务端拒绝自审、过期版本和仍有阻断项的批准。
...
讲稿编辑页已新增“原稿与修改稿对照”，比较任意两个已保存版本，显示新增、删除、正文、类型、引用及顺序变化，同时展示各自保存的依据快照。正文未变但商品或规则版本改变时另行提示。未保存的编辑不会被伪装为已存版本，也不会因查看对照改变稿件或定稿状态。独立审核角色已在开发增量中实现；逐条建议提出、采纳/拒绝、生成新稿和重新独立审核也已实现，均已随 0.6 部署。
...
| POST       | `/courses/:id/scripts/:version/confirm` | 仅限非生产、未配置团队的单人演示确认                                   |
...
| 多角色审批        | 已加入编辑/审核/主播/复盘角色和独立决定   | 账号管理界面、确认撤回及更完整审计                                     |
...
团队审核接口和生产升级约束见 [团队审核](team-review.md)。

## 长稿任务开发增量

已有大纲到分章生成、检查点恢复、取消、模型待配置状态和待审导入，已随 0.6 部署。真实模型与长稿效果待配置后验收；操作与边界见[生成任务说明](generation.md)。人工编辑和已存稿件不依赖模型服务。
