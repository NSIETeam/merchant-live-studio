...
讲稿编辑页已新增“原稿与修改稿对照”，比较任意两个已保存版本，显示新增、删除、正文、类型、引用及顺序变化，同时展示各自保存的依据快照。正文未变但商品或规则版本改变时另行提示。未保存的编辑不会被伪装为已存版本，也不会因查看对照改变稿件或定稿状态。独立审核角色已在开发增量中实现；逐条建议提出、采纳/拒绝、生成新稿和重新独立审核也已实现，均尚未部署。
...
已有大纲到分章生成、检查点恢复、取消、模型待配置状态和待审导入，尚未部署。真实模型与长稿效果待配置后验收；操作与边界见[生成任务说明](generation.md)。人工编辑和已存稿件不依赖模型服务。
