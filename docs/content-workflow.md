...
讲稿编辑页已新增“原稿与修改稿对照”，比较任意两个已保存版本，显示新增、删除、正文、类型、引用及顺序变化，同时展示各自保存的依据快照。正文未变但商品或规则版本改变时另行提示。未保存的编辑不会被伪装为已存版本，也不会因查看对照改变稿件或定稿状态。独立审核角色已在开发增量中实现；逐条建议处置仍在完整交付清单中。
...
| 多角色审批        | 已加入编辑/审核/主播/复盘角色和独立决定   | 账号管理界面、逐条建议处置、集中审批队列、确认撤回及更完整审计         |
