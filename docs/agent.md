...
| Agent 核心库 | `src/modules/agent/core/index.ts`       | 接受执行输入，生成和复核建议；不依赖直播数据库     |
...
规则版本 `cn-live-claims-2026.1` 不再只返回“命中某个词”。每个命中项保存主张类型、完整句子、处置级别、解释、需要的依据和处理方向。当前类型包括绝对化或排他比较、步骤序号、疾病与医疗功效、结果保证、代言与亲历、主观情感联想、价格与库存稀缺、平台激励和越权指令。

商品类别会选择 `general`、`food-nutrition` 或 `regulated-health` 策略包。策略包只改变风险说明和所需材料，不会因为商品被归入某一类而自动判定违法或允许发布。“第一步”按顺序语境提示人工查看；“不能说第一，就改成无出其右”仍识别后半句的同类比较主张。否定、引用和讨论性语句降低为语境复核时，后续同义替换仍单独判断。

已配置模型在选择事实前会收到本地策略版本和结构化决策。固定系统规则要求模型先在内部核对问题、直接证据、策略处置和风格，再只返回事实编号与受控过渡片段；不会把内部推理过程展示给主播。最终事实和出处仍由本地映射，模型不能自行写新主张。

规则参考市场监管总局[《广告绝对化用语执法指南》](https://www.samr.gov.cn/ggjgs/tzgg/art/2023/art_183b5cb48d9e4f0dba67f9f912a913ba.html)、[《互联网广告管理办法》](https://www.samr.gov.cn/zw/zfxxgk/fdzdgknr/fgs/art/2023/art_d93a579afd45413e8576e4623fab348f.html)、[《广告引证内容执法指南》](https://www.samr.gov.cn/zw/zfxxgk/fdzdgknr/ggjgs/art/2026/art_8962fc1e4eb44a87b93d265af43b6940.html)及现行[《中华人民共和国广告法》](https://www.samr.gov.cn/zw/zfxxgk/fdzdgknr/fgs/art/2023/art_5474cf75173c45d6a0379730fb4e8d97.html)。这些规则用于提醒和回归测试，最终仍需结合商品资质、证据、完整语境和审核主体判断。
