...
通过浏览器保存“本地合成验收 · 六项表达边界”，使用标准风格 V1 的规则模式运行六项：比较性承诺、情感泛化、资料不足、已有依据引用、越权指令、治疗承诺。任务全部完成，6 项满足模板预设检查，0 项待调整/等待；报告保留示例事实快照和规则模式标识。重新加载页面后仍能从历史评测读取同一结果。

完整展开时手机页面约 7188px，六个场景分别占 674–998px。现在每个场景默认收为摘要，显示各方案状态，点击后查看建议、证据、检查和评审。390px 视口下摘要各约 69px、横向溢出 0，展开后建议内容可见；已实际核对展开和收起。不同报告使用独立键，避免复用上一报告的展开状态。

该结果不代表模型质量或人工审核通过。未配置真实模型，尚未进行品牌授权样例的风格评判。
