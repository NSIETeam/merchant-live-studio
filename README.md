...
**真实模型稍后配置。** 当前支持粘贴/UTF-8 文字导入整稿、逐段手工编辑、结构化语义风险提示和人工定稿；0.6 已加入成员角色、集中待审、逐条修改建议和独立审稿流程；长稿任务已支持大纲、分章生成、失败续写和待审导入，详见[生成任务说明](docs/generation.md)。真实模型与主播风格效果尚未验收，默认 `grounded-rules` 不执行长稿生成。实时转写已提供供应方无关的最终分段 webhook、持久队列和有界 Agent 联动，但真实 ASR 服务尚未配置。微信公众号 OAuth 身份接口已实现但默认关闭，尚未使用获授权公众号实测；微信支付、收款授权与门店订单归因仍未接入，红包只做模拟处理。
...
| Agent | 独立进程/库、标准与品牌风格、版本/试演/发布、异步建议、结构化主张复核、证据和阶段摘要、反馈、语音分段联动 | 真实模型与 ASR 服务配置、效果调试、持续扩充行业评测语料 |
