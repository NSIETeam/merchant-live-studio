...
**真实模型稍后配置。** 当前支持粘贴/UTF-8 文字导入整稿、逐段手工编辑、有限规则检查和人工定稿；开发增量已加入成员角色和独立审稿流程；尚未实现 AI 长稿生成或主播风格学习。现有 Agent 模型接口用于短句建议，默认 `grounded-rules`，不会冒充真实模型推理。微信身份与支付、实时 ASR、门店订单归因尚未接入，红包只做模拟处理。
...
| 模块  | 当前可运行行为                                                                            | 后续接入                                       |
| ----- | ----------------------------------------------------------------------------------------- | ---------------------------------------------- |
| 内容  | 独立商品/SKU与证据版本、周期/课次/课时、长稿编辑/引用、规则定位、人工定稿、房间绑定与播讲 | AI长稿生成、逐条建议处置、主播档案、线下归因   |
