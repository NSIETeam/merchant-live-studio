...
import {
  findAgentProfilesByTenantId,
  findAgentProfilesByTenantIdAndId,
  findAgentPromptVersionsByTenantIdAndProfileIdAndVersion,
  findAgentRuns,
  findAgentRuns2,
  findAgentRunsByTenantId,
  findAgentRunsByTenantIdAndId,
  findAgentRunsByTenantIdAndIdempotencyKey,
  insertAgentProfiles,
  insertAgentProfiles2,
  insertAgentPromptVersions,
  insertAgentPromptVersions2,
  insertAgentPromptVersions3,
  insertAgentRuns,
  listAgentProfilesByTenantId,
  listAgentPromptVersionsByTenantIdAndProfileId,
  listAgentRuns,
  listAgentRunsByTenantIdAndRoomIdAndMode,
  updateAgentProfilesByTenantIdAndId,
  updateAgentProfilesByTenantIdAndId2,
  updateAgentRunsByTenantIdAndId,
  updateAgentRunsByTenantIdAndId2,
  updateAgentRunsByTenantIdAndId3,
  updateAgentRunsByTenantIdAndId4,
} from "./persistence/app-queries.js";
import { agentTransaction, type AgentDB } from "./persistence/database.js";
...
export function createAgentService(
  ...
  const version = (
    ...
    const row = findAgentPromptVersionsByTenantIdAndProfileIdAndVersion(
      db,
      tenant,
      id,
      number,
  ...
  const run = (tenant: string, id: string): RunRow => {
    const row = findAgentRunsByTenantIdAndId(db, tenant, id) as
      RunRow | undefined;
  ...
  const seed = (tenant: string) =>
    agentTransaction(db, () => {
      if (findAgentProfilesByTenantId(db, tenant)) return;
      const now = clock();
      insertAgentProfiles(db, tenant, "标准直播风格", now);
      insertAgentPromptVersions(
        db,
        tenant,
        JSON.stringify(DEFAULT_PROMPT_CONTENT),
        now,
      );
    });
  function status(): AgentServiceStatus {
    try {
      const rows = listAgentRuns(db) as { status: string; count: number }[];
      return {
    ...
    while (active.size < config.concurrency) {
      ...
      try {
        next = agentTransaction(db, () => {
          const row = findAgentRuns2(db) as RunRow | undefined;
          if (row)
            updateAgentRunsByTenantIdAndId(db, clock(), row.tenant_id, row.id);
          return row;
        });
      } catch {
        return;
  ...
  app.get("/v1/profiles", (c) =>
    c.json({
      profiles: (
        listAgentProfilesByTenantId(db, c.get("tenantId")) as ProfileRow[]
      ).map(profileDto),
    }),
  );
  app.post("/v1/profiles", async (c) => {
    ...
    const { name, kind, ...content } = input,
      ...
      now = clock();
    agentTransaction(db, () => {
      insertAgentProfiles2(db, tenant, id, name, kind, now);
      insertAgentPromptVersions2(db, tenant, id, JSON.stringify(content), now);
    });
    return c.json(
      {
  ...
  app.post("/v1/profiles/:id/versions", async (c) => {
    ...
    const number = agentTransaction(db, () => {
      ...
      updateAgentProfilesByTenantIdAndId(db, next, tenant, id);
      return next;
    });
  ...
  app.post("/v1/runs", async (c) => {
    ...
    const id = agentTransaction(db, () => {
      const prior = findAgentRunsByTenantIdAndIdempotencyKey(
        db,
        tenant,
  ...
  app.post("/v1/runs/:id/feedback", async (c) => {
    ...
    const tenant = c.get("tenantId"),
      id = c.req.param("id");
    run(tenant, id);
    updateAgentRunsByTenantIdAndId4(db, JSON.stringify(feedback), tenant, id);
