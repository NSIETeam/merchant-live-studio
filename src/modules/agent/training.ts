...
import {
  findAgentEvaluationSuitesByTenantIdAndId,
  findAgentEvaluationSuitesByTenantIdAndRoomIdAndName,
  findAgentEvaluationsByTenantIdAndId,
  findAgentEvaluationsByTenantIdAndIdempotencyKey,
  findAgentExampleImportsByTenantIdAndProfileIdAndIdempotencyKey,
  insertAgentEvaluationItems,
  insertAgentEvaluationSuites,
  insertAgentEvaluations,
  insertAgentExampleImports,
  insertAgentPromptVersions,
  listAgentEvaluationItemsByTenantIdAndEvaluationId,
  listAgentEvaluationSuitesByTenantIdAndRoomId,
  listAgentEvaluationsByTenantIdAndRoomId,
  listAgentExampleImportsByTenantIdAndProfileId,
  updateAgentEvaluationItemsByTenantIdAndEvaluationIdAndId,
  updateAgentProfilesByTenantIdAndId,
} from "./persistence/training-queries.js";
...
export function registerTrainingRoutes(
  ...
  const suite = (tenant: string, id: string) => {
    const row = findAgentEvaluationSuitesByTenantIdAndId(db, tenant, id) as
      SuiteRow | undefined;
    if (!row)
  ...
  const report = (tenant: string, id: string): EvaluationReport => {
    const row = findAgentEvaluationsByTenantIdAndId(db, tenant, id) as
  ...
  app.post("/v1/profiles/:id/examples/import", async (c) => {
    ...
    const receipt = agentTransaction(db, () => {
      ...
      const prior =
        ...
        ) as { request_digest: string; receipt_json: string } | undefined;
      if (prior) {
  ...
  app.get("/v1/profiles/:id/example-imports", (c) => {
    ...
    return c.json({
      imports: (
        listAgentExampleImportsByTenantIdAndProfileId(db, tenant, id) as {
          receipt_json: string;
        }[]
  ...
  app.post("/v1/evaluations", async (c) => {
    ...
    const id = agentTransaction(db, () => {
      const prior = findAgentEvaluationsByTenantIdAndIdempotencyKey(
        db,
        tenant,
        input.idempotencyKey,
  ...
  app.get("/v1/evaluations", (c) => {
    const query = z
      ...
      .parse(c.req.query());
    const tenant = c.get("tenantId"),
      rows = listAgentEvaluationsByTenantIdAndRoomId(
        db,
        tenant,
        query.roomId,
        query.limit,
