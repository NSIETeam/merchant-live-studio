...
import {
  findMaterialImportsByRoomIdAndIdempotencyKey,
  insertFacts,
  insertMaterialImports,
  listMaterialImportsByRoomId,
} from "./persistence/materials-queries.js";
...
export function attachMaterials(
  ...
  app.get("/api/merchant/rooms/:id/materials", (c) => {
    ...
    const batches = listMaterialImportsByRoomId(db, roomId) as BatchRow[];
  ...
  app.post("/api/merchant/rooms/:id/materials/import", async (c) => {
    ...
    const batch = transaction(db, () => {
      const prior = findMaterialImportsByRoomIdAndIdempotencyKey(
        db,
        roomId,
        idempotencyKey,
      ...
      for (const r of parsed.rows) {
        ...
        const id = randomUUID();
        insertFacts(db, id, roomId, r.text, r.evidence);
        factIds.push(id);
      }
      const id = randomUUID(),
        createdAt = clock();
