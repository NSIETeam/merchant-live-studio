import { recordingQuery1, recordingQuery2, recordingQuery3, recordingQuery4, recordingQuery5, recordingQuery6, recordingQuery7, recordingQuery8, recordingQuery9, recordingQuery10 } from "./persistence/recording-queries.js";
import type { Hono } from "hono";
import { HTTPException } from "hono/http-exception";
import { randomUUID, createHash } from "node:crypto";
import { z } from "zod";
import type { Config, createRecordingStorage } from "../../platform/infrastructure/public.js";
      ...
      try {
        ...
        transaction(db, () => {
          ...
          if (old) {
            ...
            recordingQuery3(db, randomUUID(), room.merchant_id, item.roomId, item.relativePath, item.startedAt, item.completedAt, item.durationSeconds, verified.bytes, verified.sha256, Date.now());
          recordingQuery4(db, name);
        });
        await storage.processed(name);
      } catch (error) {
        recordingQuery5(db, name, merchantId, error instanceof Error ? error.message : "录像登记失败", Date.now());
