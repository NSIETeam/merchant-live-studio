...
import {
  findContentScriptSuggestionsById,
  findContentSuggestionResolutionsBySuggestionId,
  insertContentScriptSuggestions,
  insertContentSuggestionResolutions,
  listContentScriptSuggestionsByCourseIdAndScriptVersion,
} from "./persistence/script-suggestions-queries.js";
...
export function attachScriptSuggestions(
  ...
  const list = (id: string, version: number) =>
    listContentScriptSuggestionsByCourseIdAndScriptVersion(db, id, version);
  app.get(base, (c) => {
  ...
  app.post(base, async (c) => {
    ...
    transaction(db, () => {
      ...
      const existing = findContentScriptSuggestionsById(db, input.id);
      if (existing) {
        if (
      ...
      const review = readScriptReview(db, id, version, content);
      ...
      if ({ v: content.latest(id) }!.v !== version)
  ...
  app.post("/api/merchant/content/suggestions/:id/resolve", async (c) => {
    ...
    const result = transaction(db, () => {
      const suggestion = findContentScriptSuggestionsById(db, id);
      ...
      const existing = findContentSuggestionResolutionsBySuggestionId(db, id);
      if (existing) {
        if (
      ...
      if (input.decision === "accepted") {
        const latest = Number({ v: content.latest(course) }!.v);
